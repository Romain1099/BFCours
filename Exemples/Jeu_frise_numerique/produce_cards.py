...
# Exemple de contenu LaTeX simulé
latex_content = r"""
...
# Expression régulière pour capturer les exercices et leur contenu
exo_pattern = re.compile(
    r'\\rdifficulty\{(\d+)\}'  # Capturer la difficulté (1, 2, 3)
    r'.*?\\begin\{EXO\}.*?'    # Début de l'exercice
    r'\\begin\{enumerate\}(.*?)\\end\{enumerate\}'  # Capturer les questions
    r'.*?\\exocorrection.*?'   # Séparation
    r'\\begin\{enumerate\}(.*?)\\end\{enumerate\}', # Capturer les solutions
    re.DOTALL
)

# Stockage des cartes par difficulté
cards_by_difficulty = {1: [], 2: [], 3: []}
# Mapping des couleurs selon la difficulté
color_mapping = {
    3: "red",
    2: "blue",
    1: "green"
...
# Fonction pour diviser une liste en sous-listes de taille fixe
def split_list(lst, size):
    return [lst[i:i + size] for i in range(0, len(lst), size)]

# Génération du code LaTeX pour chaque difficulté, en respectant 9 cartes par page
latex_output = ""
# Traitement des exercices
for match in exo_pattern.findall(latex_content):
    difficulty = match[0]
    questions = re.findall(r'\\item\s+\\itempoint\{\d+\}(.*)', match[1].strip())
    solutions = re.findall(r'\\item\s+(.*)', match[2].strip())

    for idx, (question, solution) in enumerate(zip(questions, solutions), start=1):
        color = color_mapping.get(int(difficulty), "Gray")
        latex_output += rf"""\mescartes{{{color}}}{{Q.{difficulty}.{idx}}}{{{question.strip().replace(r'\begin{minipage}[t]{\linewidth}','').replace(r'\end{minipage}','')}}}{{S.{difficulty}.{idx}}}{{{solution.strip().replace(r'\begin{minipage}[t]{\linewidth}','').replace(r'\end{minipage}','')}}}%
"""


# Ajout d'un saut de page après chaque ensemble de 4 paires (2 lignes)
formatted_output = ""
cards_per_row = 1  # Deux paires question/réponse par ligne

split_output = latex_output.strip().split("\n")
for i in range(0, len(split_output), cards_per_row * 2):
    formatted_output += "\n".join(split_output[i:i + cards_per_row * 2]) + "\n\\vspace{1cm}\n"
...
'''
for diff, cards in cards_by_difficulty.items():
    if cards:
        card_chunks = split_list(cards, 9)  # Diviser en pages de 9 cartes max

        for chunk in card_chunks:
            cards_str = "\n".join(chunk)
            latex_output += rf"""
\Cartes[BackgroundAr,Loop=false,Trame,Couleur={color_mapping[diff]}]{{
{cards_str}
}}
\newpage
"""
