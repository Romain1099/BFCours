...
\def\difficulty{1}
\chapEval[
    $\mathbf{4^{\text{ème}}}$% niveau : $\mathbf{6^{\text{ème}}}$,$\mathbf{5^{\text{ème}}}$,$\mathbf{4^{\text{ème}}}$,$\mathbf{3^{\text{ème}}}$,$\mathbf{2^{\text{nde}}}$,$\mathbf{1^{\text{ère}}}$,$\mathbf{T^{\text{Le}}}$,
    ]{
    Calcul littéral% theme : ,Equations
    }{
    Collège% type_etablissement : Collège,Lycée
    }{
    Gaston Bachelard% nom_etablissement : Othe et Vanne,Amadis Jamyn,Eugène Belgrand
    }{
    \tableauEval{15}%,30},55}%}
    }{
    Devoir :
